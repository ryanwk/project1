...
/***/ (function(module, exports, __webpack_require__) {
	...
	var signUpRequest = function signUpRequest(data) {
	  return $.ajax({
	    ...
	    method: 'POST',
	    data: data
	  }).done(ui.signUpSuccess).fail(ui.signUpFailure);
	};
	var signInRequest = function signInRequest(data) {
	  return $.ajax({
	    ...
	    method: 'POST',
	    data: data
	  }).done(ui.signInSuccess).fail(ui.signInFail);
	};
	var signOut = function signOut() {
	  ...
	  }).done(ui.signOutSuccess).fail(ui.failure);
	};
	var createGame = function createGame(data) {
	  ...
	  return $.ajax({
	    ...
	    data: data
	  }).done(ui.createGameSuccess).fail(ui.failure);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var ui = __webpack_require__(7);
	var gameStats = __webpack_require__(10);
	var getFormFields = __webpack_require__(3);
	var config = __webpack_require__(11);
	var store = __webpack_require__(8);
	...
	var resetBoardAfterWin = function resetBoardAfterWin() {
	  xTurn = true;
	  gameState = [0, 0, 0, 0, 0, 0, 0, 0, 0];
	  turnCounter = 0;
	  gameOver = true;
	...
	var resetBoardDuringGame = function resetBoardDuringGame() {
	  if (gameOver === false) {
	    xTurn = true;
	    gameState = [0, 0, 0, 0, 0, 0, 0, 0, 0];
	    turnCounter = 0;
	...
	var toggleTurn = function toggleTurn(event) {
	  ...
	  if (xTurn) {
	    ...
	    if (checkForWin(1)) {
	      ...
	      resetBoardAfterWin();
	      gameOver = true;
	      updateServer(letter, gameOver, index).done(updateServerSuccess).catch(updateServerFailure);
	      return;
	    } else {
	      gameOver = false;
	      updateServer(letter, gameOver, index).done(updateServerSuccess).catch(updateServerFailure);
	...
	var updateServer = function updateServer(position, player, status) {
	...
	var updateServerSuccess = function updateServerSuccess() {};
	var updateServerFailure = function updateServerFailure() {};
	// end updateServer Patch request

	// this function checks if there are any combination of 3 letters in a row and
	// determines a winner
	var checkForWin = function checkForWin(xIndicator) {
	  // Check diagonals for wins
	  if (gameState[0] === xIndicator && gameState[4] === xIndicator && gameState[8] === xIndicator) {
	    return true;
	  } else if (gameState[2] === xIndicator && gameState[4] === xIndicator && gameState[6] === xIndicator) {
	    return true;
	  }
	  // Check horizontal and vertical columns for wins
	  for (var i = 0; i < 3; i++) {
	    // Check horizontal columns for win
	    if (gameState[i * 3] === xIndicator && gameState[i * 3 + 1] === xIndicator && gameState[i * 3 + 2] === xIndicator) {
	      return true;
	    } else if (gameState[i] === xIndicator && gameState[i + 3] === xIndicator && gameState[i + 6] === xIndicator) {
	      return true;
	    }
	  }
	  return false;
	...
	var onChangePassword = function onChangePassword(event) {
	  ...
	  return $.ajax({
	    ...
	    data: data
	  }).done(ui.changePasswordSuccess).fail(ui.changePasswordFailure);
	};
	...
	module.exports = {
	  ...
	  resetBoardDuringGame: resetBoardDuringGame,
	  index: index,
	  letter: letter,
	  gameOver: gameOver,
	  onChangePassword: onChangePassword,
	  updateServer: updateServer,
	  updateServerSuccess: updateServerSuccess,
	  updateServerFailure: updateServerFailure
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var resetGameStatusVar = function resetGameStatusVar() {
	...
	module.exports = {
	  ...
	  resetGameStatusVar: resetGameStatusVar,
	  getGameStatus: getGameStatus,
	  changePasswordFailure: changePasswordFailure,
	  changePasswordSuccess: changePasswordSuccess,
	  modalEscapeSignUp: modalEscapeSignUp,
	  modalEscapeSignIn: modalEscapeSignIn,
	  changePasswordEscape: changePasswordEscape
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var startGameView = function startGameView() {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var gameStatsUpdate = function gameStatsUpdate(data) {
	  return $.ajax({
	    url: config.apiOrigin + '/games',
	    method: 'GET',
	    headers: {
	  ...
	  }).done(gameStatsSuccess).catch(gameStatsFail);
	};
	...
	var gameStatsFail = function gameStatsFail() {};

	module.exports = {
	  gameStatsUpdate: gameStatsUpdate
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
